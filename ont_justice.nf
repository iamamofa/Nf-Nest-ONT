...
workflow {
 ...
 // Process execution
    ...
    trimAdapters(concatenated_fastq)
    qualityControl(trimmed_out)
    alignReads(trimmed_out)
    callVariants(aligned_out)
    processAlignments(aligned_out)
    getcoverage(postal_out6)
    generateCoveragePlots(coverage_plots)
    filterLowCoverage(low_coverage_filter, depth_stats)
    generateFinalReport(coverage, depth, filtered_regions_out)
    generateVCF(postal_out6)
    annotate(vcf_out)
    vartypes(vartype_out)
    merge(merge_out)
    filter(filter_out)
    extract(extract_out)
    spread(spread_out)
    snpfilter(snpfilter_out)
    tojson(tojson_out)
    getcutoff(list, list1)
    viz(logpath1)
    errortrack(logpath1, workpath, CTexcelpath)
}
...
process concatenateFastq {
    ...
    input:
        path barcode_folder from Channel.fromPath("${params.input.fastq_path}/*", type: 'dir')
...
process trimAdapters {
    ...
    input:
        set val(barcode), path(fastq_group)
...
process qualityControl {
    ...
    input:
        tuple val(barcode), path(fastq) from trimmed_out
...
process alignReads {
    ...
    input:
        set val(barcode), path(fastq) from trimmed_out
...
process getcoverage {
    ...
    input:
        set val(barcode), path(bam) from postal_out 
...
process generateCoveragePlots {
    ...
    output:
        file "${barcode}_coverage_plot.png"

    script:
        """
        python3 ${pyscripts}/generate_coverage_plot.py -i ${coverage_file} -o ${barcode}_coverage_plot.png
        """
}
...
process vartypes {
    ...
    input:
        set val(barcode), path(vcf) from anno_out
...
process merge {
    ...
    input:
        set val(barcode), path(vartype_vcf) from vartypes_out
        set val(barcode), path(bam) from postal_out
